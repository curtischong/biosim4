...
void initializeGeneration0WithGenome(Genome genome)
{
    ...
    for (uint16_t index = 1; index <= p.population; ++index) {
        if (genome.size() == 0){
            ...
            peeps[index].initialize(index, grid.findEmptyLocation(), Genome{genome});
        }
    }
}
    ...
    {
        initializeGeneration0WithGenome(Genome{});
    }
